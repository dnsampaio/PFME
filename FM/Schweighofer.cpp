...
SchweighoferTester::SchweighoferTester(exset ineqs, unsigned d) :
    numSrcs(ineqs.size()), problem(nullptr), monomPos({}), ineqPos({}), order(0), MAX_ORDER(std::max(1u, d)){
...
SchweighoferTester::~SchweighoferTester(){
  clear();
}
...
bool SchweighoferTester::goodNumbers(const ex &compareTo) const{
  ...
  for ( int s = 2, e = glp_get_num_cols(problem); s <= e; s++ ){
    if ( glp_get_col_prim(problem, s) < - MAX_ERROR ){
      DEBUG(4, "FIXME!!! GLP getting col_prim[" << s <<"] == " << glp_get_col_prim(problem, s) << NL);
      return false;
    }
  }
...
//  cerr << __FILENAME__ << "::" << __func__ << "::" << __LINE__ << "::FIX ME!!!\n\tTesting if " << compareTo << GE << "0, we got that " << res << GE << "0; difference is: " << final
...
bool SchweighoferTester::isProved(int o, bool isExact, const ex &compareTo) const{
  if ( o != 0 ){
    switch ( o ){
      case GLP_EBADB:
        DEBUG(8,
            "GLP_EBADB: Unable to start the search, because the initial\nbasis specified in the problem object is invalid—the number of\nbasic (auxiliary and structural) variables is not the same as\nthe number of rows in the problem object.\n")
        ;
      break;
      case GLP_ESING:
        DEBUG(8, "GLP_ESING: Unable to start the search, because the basis\nmatrix corresponding to the initial basis is exactly singular.\n")
        ;
      break;
      case GLP_EBOUND:
        DEBUG(8, "GLP_EBOUND: Unable to start the search, because some double-bounded\n(auxiliary or structural) variables have incorrect bounds.\n")
        ;
      break;
      case GLP_EFAIL: {
        DEBUG(8, "GLP_EFAIL The problem instance has no rows/columns.\n");
        if ( !isExact )
          return true; //Hack to handle GLPK bug: https://lists.gnu.org/archive/html/bug-glpk/2016-02/msg00002.html
        assertM(false, "Why???\n");
        break;
      }
      case GLP_EITLIM:
        DEBUG(8, "GLP_EITLIM: The search was prematurely terminated, because\nthe simplex iteration limit has been exceeded.\n")
        ;
        return goodNumbers(compareTo);
      break;
      case GLP_ETMLIM:
        DEBUG(8, "The search was prematurely terminated, because the time limit\nhas been exceeded.\n")
        ;
        return goodNumbers(compareTo);
      break;
      default:
...
testResult SchweighoferTester::test_factorized(ex ineq){
  ...
  if ( is_a<power>(ineq) ){
    ...
    assertM(is_a<numeric>(p.op(1)), "Power " << p << " has non numeric exponent!!\n");
    numeric exp = ex_to<numeric>(p.op(1));
    assertM(exp.is_integer(), "Power " << p << " has non integer exponent " << exp << "!!\n");
    assertM(exp.is_nonneg_integer(), "Power " << p << " has negative integer exponent " << exp << "!!\n");
...
testResult SchweighoferTester::test(ex ineq, bool testPosAndNeg){
  ...
  if ( is_a<numeric>(ineq) ){
    ...
    DEBUG(6, "Testing if we have the expression being tested, by a different constant\n");
  ...
  for ( const auto &mon : monomialsCoeffs ){
    ...
    DEBUG(7, "glp_set_row_bnds: " << it->second << '(' << it->first << ") == " << mon.second << "\n");
...
//  config.tol_bnd = 1.0e-9; //Tolerance used to check if the basic solution is primal feasible.
//  config.tol_piv = 1.0e-11; //Tolerance used to choose eligble pivotal elements of the simplex table.
  config.it_lim = 10000;
  config.tm_lim = 600;
...
//    glp_write_prob(problem, 0, "problem.txt");
    ...
    if ( !isProved(o, true, ineq) ){
      ...
      else assert(false); //??? Forced it to be >= -0.9999999
      if ( isNumeric ){ //If it was proved E >= 0, and E is a numeric, then distance == E, else it is an absurd! (or bug???)
        if ( colPrim != result.distance ){
          cerr << __FILENAME__ << "::" << __func__ << "::" << __LINE__ << ":: Proved that" << ineq << " is " << result << NL;
  ...
  if ( ( !testPosAndNeg ) or isNumeric ){ //It was requested to verify E >= 0 and -E >= 0?
    if ( result.sign == SIGN::UNKNOWN )
      return test_factorized(ineq);
...
void SchweighoferTester::gatherMonomials(const ex &ineq, gexmap<double> &coeffs){
  if ( is_a<numeric>(ineq) ){
  ...
  DEBUGFOR(9, const auto &sub : coeffs, ineq << " is a add monomial " << sub.second << " * " << sub.first << "\n");
}

//TODO: 1: Remove constant from ineq map, ex: 4x + 3yz -2 >= 0; remove the numerical [ -2 ] value.
//2: Keep map of these new expressions to their relative numeric value: map[4x + 3yz] = -2
//3: When testing 4x + 3yz -1, we also decompose as [4x + 3yz][-1]. As the tester has that map[[4x + 3yz] = -2 < -1, our starting test value is already {GTZ, 1}.
...
void SchweighoferTester::expandSrcs(const exset &root, idx_map &indexes){
  ...
  //Order N: S^N = S * S ^ (N-1); Multiply the system by the last generated degree to obtain the next one
      ...
      for ( const ex &prevOrd : expanded[order - 1] ){
        ...
        for ( const auto atOrder : expanded ){
          if ( atOrder.second.find(expanExp) != atOrder.second.end() ){
            DEBUG(8, expanExp << " is already existent at degree " << atOrder.first)
            skip = true;
            break;
          }
        }
...
ostream &SchweighoferTester::printResult(ostream &o, const bool printSteps) const{
  ex res = 0;
  string front = "   ";
  for ( const auto &col : ineqPos ){
    double c = glp_get_col_prim(problem, col.second);
