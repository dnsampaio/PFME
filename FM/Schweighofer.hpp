...
/* The SchweighoferTester implements a positiveness test. For a given system of
 * non-negative inequality constraints S = {E0 >= 0, E1 >= 0 ... En >= 0},
 * multiplying and adding their expressions always generate non-negative
 * inequalities (E0^n*E^m >= 0). If an expression can be written by combining
 * any of these inequalities, multiplied by non-negative values, then the S
 * implies E' is positive. The obvious inequality 1 >= 0 is also part of the
 * system of constraints. If, in the end, the system can generate that E' =
 * ADD(MUL(E0...En)) + c*(1), where c >= 0, it means that E' is implied to be
 * positive by a distance of c. To determine each expression coefficient,
 * constrainted to non-negative values, this problem is formulated as a linear
 * problem. Each row of the problem represent an monomial and each column an
 * expression E(0..n). Each cell of the table tells the index of that monomial
 * in the given expression. The rhs column holds the coefficients of each
 * monomial for the evaluated expression E' (or -E'), as a fixed value (LHS*x =
 * RHS). Each variable (column) must be non-negative. And our objective function
 * is to maximaze x0, the coefficient that multiplies "1".
 *
 * Our tester evaluates both E' and -E' to determinate if either positive or
 * negative sign is implied by the system. */
...
struct SchweighoferTester {
  ...
  const exPos &getIneqs() const { return ineqPos; }

  const exPos &getMonomials() const { return monomPos; }
...
private:
  ...
  typedef map<int, map<int, double>>
      idx_map; // Maps monomials coefficients per expression. In Column x Row
               // format, that is: map[Inequality|Column][Monomial|Row] =
               // coefficient
