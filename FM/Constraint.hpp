...
public:
  ...
  static cc getFalse() { return &falseC; }
  static cc getTrue() { return &trueC; }
  static cs getOrInsertSymbol(const std::string &s) {
    ...
    if (sy == SymTab.end()) {
      DEBUG(10, "Creating new symbol " << s << NL);
      SymTab[s] = GiNaC::symbol(s);
      return SymTab[s];
    }
    DEBUG(10, "Reusing symbol " << s << NL);
    return sy->second;
  }
  static cc get(std::string buf);
  ...
  const Constraint *getNot() const { return get(-id); }
