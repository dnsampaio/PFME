...
Constraint::cc Constraint::get(const ex &expr, bool isEq) {
  ...
  for (auto const &ct : constraints) {
    ...
    if ((isEq == c.eq) and
        ((c.exp.is_equal(exp)) or (isEq and (c.exp.is_equal(-exp))))) {
      return &c;
    }
  }
...
Constraint::cc Constraint::get(std::string buf) {
  ...
  if (buf.substr(0, 2) == "eq") {
    ...
    case SIGN::ABSURD:
    case SIGN::UNKNOWN: {
      cerr << "Something went wrong when generating constraint from string "
           << buf << NL << rhs << op << lhs << NL;
      exit(BAD_INPUT);
    }
    case SIGN::GEZ: {
      x = expand(lhs - rhs);
...
bool Constraint::obvious(const ex &exp, bool eq) {
  if (eq)
    ...
    return is_a<numeric>(exp) and !ex_to<numeric>(exp).is_negative();
}
...
ostream &Constraint::print(ostream &out) const {
  ...
  if (cerrbuf == out.rdbuf())
    return (out << '[' << setw(2 + (int)(floor(log10(nId)))) << id << "] "
                << exp << ((eq) ? ((id < 0) ? NE : " = ") : GE) << "0");
...
ostream &Constraint::c_print(ostream &out) const {
  ...
  /* Convert e1 - e2 + e3 - e4 + c >= 0  ==============>  e1 - e2 + e3 - e4 >=
   * -c */
  ...
  if (is_a<add>(lhsExp)) {
    ...
    for (const ex &op : lhsExp) {
      ...
      if (is_a<numeric>(op) and (op < 0)) {
        rhsExp -= op;
        DEBUG(10, op << " is a negative number operand, adding inverse to "
                        "rhsExp becoming: "
                     << rhsExp << NL);
        continue;
      }
      if (is_a<mul>(op)) {
        ...
        if (invert) {
          rhsExp -= op;
          DEBUG(10, op << " is a negative multiplied operand, such that rhsExp "
                          "becomes: "
                       << rhsExp << NL);
        } else {
          DEBUG(10, op << " is NOT a negative multiplied operand, such that "
                          "rhsExp becomes: "
                       << rhsExp << NL);
        }
        continue;
  ...
  DEBUG(10, exp << " ------------- becomes ---->" << lhsS.str() << oper1 << "="
                << rhsS.str() << NL);
