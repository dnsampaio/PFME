...
void pp(const sysType &s, const unsigned &c) {
  ...
  for (const cst &e : s)
    cout << '\t' << '[' << setw(space) << e.second << "] " << e.first << GE << 0
         << NL;
  cout << "}\n";
}
  ...
  while (!cin.eof()) {
    if (do_print) {
      cout << "Commands: (A)ppend one constraint; E(x)it;\n\t(P)rint "
              "system;(C)hange ST degree [current="
           << degree << "];\n";
      if (sys.size())
        cout << "\t(T)est if an expression is implied;\n\t(D)elete one "
                "constraint; Obtaint (R)oot of the system;\n\t(S)how ST "
                "expressions; Show ST (m)onomials;\n";
    }
    do_print = true;
    fflush(stdin);
    string s;
    ...
    case 'A': {
      ...
      cout << "Constraint [" << it->second << "] " << it->first
    ...
    case 'R': {
      ...
      while (minSize < maxSize) {
        ...
        for (const auto &nc : constraintsBySize[minSize]) {
          ...
          case SIGN::ZERO: {
            removed++;
            DEBUG(7, "Remove the constraint "
                         << nc.first << " as it is implied to be " << tr << NL);
            continue;
          }
          default: {
            DEBUG(7, "The constraint " << c << " is not implied " << tr
