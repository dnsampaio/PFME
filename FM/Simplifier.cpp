...
#else
#define returnAbsurd();{ messageM("Made the system false here"); conju = Conjunction::absurd; ret.push_back(Conjunction::absurd); }while(0)
#endif

#define returnObvious(){ cout << Conjunction::obvious << NL; exit(0);}
#define sanityCheck() {if ( ( Conjunction::absurd == conju ) || conju.hasInverseConstraints()){ returnAbsurd(); return;} else if ( Conjunction::obvious == conju ){returnObvious(); return;}}
...
void Simplifier::split_space(const exset &pars){
  //If there are unknown or non strict relationship between 2 of these expressions, split space
  for ( exset::const_iterator pEnd = pars.end(), p1 = pars.begin(); p1 != pEnd; p1++ ){
...
Simplifier::relat Simplifier::put_in_evidence(ex exp, const exset &vars) const{
   ...
   * 1st try to take highest degree and factorize. If you get an multiplication: f*g=h, test if at least 2 of (f,g,h) have known signs
   * 2nd try to move reminder from all variables to other side. Try to factorize one side. If successful test signs just as 1st step.
   *  If not ok, try other side.
...
bool Simplifier::eqs_affinize(){
  //The 2 most common equalities are: 1) linearized arrays 2) Packed symmetric arrays. what patterns do they follow?
  bool changed = false;
  ...
  for ( cc eq : conju.eqs ){
    ...
    DEBUG(4, '(' << factorized.evidence << ") * (" << factorized.quo << ") = " << factorized.rhs << NL);
    ...
    if ( sRHS == SIGN::ZERO ){ //RHS = 0 ==> QUO = 0 or evidence = 0
      ...
      if ( ( sQUO & SIGN::ZERO ) == 0 ){
        ...
        changed |= conju.removeJoin({}, {Constraint::get(factorized.evidence, true)});
      }
      if ( ( sQUO & SIGN::ZERO ) == 0 ){
        DEBUG(4, "RHS = 0; QUO " << NE << " 0 ==> evidence = 0\n");
        changed |= conju.removeJoin({}, {Constraint::get(factorized.evidence, true)});
      ...
      if ( 0 == ( sQUO & SIGN::ZERO ) ){
        ...
        changed |= conju.removeJoin({}, {Constraint::get(factorized.quo, true)});
      }
    }
      ...
      if ( 0 == ( sRHS & SIGN::ZERO ) ){ //We have RHS == GTZ or LTZ
        switch ( sEVI ){
          ...
          case SIGN::GEZ: {
            sEVI = SIGN::GTZ;
            changed |= conju.removeJoin({}, {Constraint::get(factorized.evidence - 1)});
        ...
        if ( sRHS == SIGN::GTZ ){
          if ( sEVI == SIGN::UNKNOWN ){ // (evidence = UNKNOWN) * (QUO) = (RHS > 0)
            switch ( sQUO ){
              ...
              case SIGN::UNKNOWN: { // (evidence = UNKNOWN) * (QUO = UNKNOWN) = (RHS > 0)
                Conjunction ltzltz = conju; //Split, either evidence = QUO = ( GEZ or LTZ )
                ltzltz.removeJoin({}, {Constraint::get(-factorized.evidence - 1), Constraint::get(-factorized.quo - 1)});
                conju.removeJoin({}, {Constraint::get(factorized.evidence - 1), Constraint::get(factorized.quo - 1)});
                DEBUG(4, "Splitting:\n\t" << ltzltz << NL<< conju << NL);
#ifdef SCRIPT_CONTROL
          ...
          else if ( sQUO == SIGN::UNKNOWN ){ // (evidence != UNKNOWN) * (QUO == UNKNOWN) = (RHS > 0)
            switch ( sEVI ){
              case SIGN::LTZ: { // (evidence = LTZ) * (QUO = UNKNOWN ==> LTZ) = (RHS = GTZ)
                DEBUG(4, "evidence = LTZ; QUO = UNKNOWN ==> LTZ; RHS = GTZ\n");
                changed |= conju.removeJoin({}, {Constraint::get(-factorized.quo - 1)});
          ...
          if ( sEVI == SIGN::UNKNOWN ){ // (evidence = UNKNOWN) * (QUO) = (RHS < 0 / LTZ)
            switch ( sQUO ){
              case SIGN::LTZ: { // (evidence = UNKNOWN) * (QUO < 0) = (RHS > 0)
              ...
              case SIGN::UNKNOWN: { // (evidence = UNKNOWN) * (QUO = UNKNOWN) = (RHS < 0 / LTZ)
                Conjunction ltzgtz = conju; //Split, either evidence = QUO = ( GEZ or LTZ )
                ltzgtz.removeJoin({}, {Constraint::get(-factorized.evidence - 1), Constraint::get(factorized.quo - 1)});
                changed |= conju.removeJoin({}, {Constraint::get(factorized.evidence - 1), Constraint::get(-factorized.quo - 1)});
                DEBUG(4, "Splitting:\n\t" << ltzgtz << NL<< conju << NL);
#ifdef SCRIPT_CONTROL
          ...
          else if ( sQUO == SIGN::UNKNOWN ){ // (evidence != UNKNOWN) * (QUO == UNKNOWN) = (RHS < 0 / LTZ)
            switch ( sEVI ){
              case SIGN::LTZ: { // (evidence = LTZ) * (QUO = UNKNOWN ==> LTZ) = (RHS = LTZ)
        ...
        if ( sRHS == SIGN::GEZ ){
          if ( sEVI == SIGN::UNKNOWN ){ // (evidence = UNKNOWN) * (QUO) = (RHS > 0)
            switch ( sQUO ){
              case SIGN::LTZ: { // (evidence = UNKNOWN) * (QUO < 0) = (RHS >= 0)
                ...
                changed |= conju.removeJoin({}, {Constraint::get(-factorized.evidence)});
                DEBUG(4, "Adding that " << factorized.evidence << LE << 0 << NL<< conju << NL);
                break;
              }
              case SIGN::GTZ: {
                DEBUG(4, "evidence = UNKNOWN ==> GEZ; QUO = GTZ; RHS = GTZ\n");
                changed |= conju.removeJoin({}, {Constraint::get(factorized.evidence)});
                DEBUG(4, "Adding that " << factorized.evidence << GE << 0 << NL<< conju << NL);
                break;
              }
              default: {
                break;
            ...
            switch ( sEVI ){
              case SIGN::LTZ: { // (evidence = LTZ) * (QUO = UNKNOWN ==> LEZ) = (RHS = GEZ)
                DEBUG(4, "evidence = LTZ; QUO = UNKNOWN ==> LEZ; RHS = GTZ\n");
                changed |= conju.removeJoin({}, {Constraint::get(-factorized.quo)});
                DEBUG(4, "Adding that " << factorized.quo << LE << NL<< conju << NL);
                break;
              }
              case SIGN::GTZ: {
          ...
          if ( sEVI == SIGN::UNKNOWN ){ // (evidence = UNKNOWN) * (QUO) = (RHS <= 0 / LEZ)
            switch ( sQUO ){
              case SIGN::LTZ: { // (evidence = UNKNOWN) * (QUO < 0) = (RHS <= 0)
            ...
            switch ( sEVI ){
              case SIGN::LTZ: { // (evidence = LTZ) * (QUO = UNKNOWN ==> LTZ) = (RHS = LTZ)
                ...
                changed |= conju.removeJoin({}, {Constraint::get(factorized.quo)});
                DEBUG(4, "Adding that " << factorized.quo << GE << 0 << NL<< conju << NL);
                break;
              }
              case SIGN::GTZ: {
      ...
      if ( ( sEVI != SIGN::UNKNOWN ) && ( sQUO != SIGN::UNKNOWN ) ){
        if ( 0 == ( sEVI & sQUO & ( ~SIGN::ZERO ) ) ){ //Opposite signs of sEVI and sREM
          if ( ( sEVI | sQUO ) & SIGN::ZERO ){
            ...
            sRHS = SIGN::LTZ;
            changed |= conju.removeJoin({}, {Constraint::get(-factorized.rhs - 1)});
          }
        }
    ...
    if ( !( sRHS == SIGN::UNKNOWN || sEVI == SIGN::UNKNOWN || sQUO == SIGN::UNKNOWN ) ){
      //Is the RHS != 0 and LHS == 0 or RHS == 0 and LHS != 0?
      bool lhsEzero = ( 0 != ( ( sEVI | sQUO ) & SIGN::ZERO ) );
      bool lhsPositive = ( ( ( sEVI & SIGN::GTZ ) && ( sQUO & SIGN::GTZ ) ) || ( ( sEVI & SIGN::LTZ ) && ( sQUO & SIGN::LTZ ) ) );
      bool rhsEzero = ( 0 != ( sRHS & SIGN::ZERO ) );
      bool rhsPositive = ( 0 != ( sRHS & SIGN::GTZ ) );
      if ( rhsPositive != lhsPositive ){
        if ( !( rhsEzero && lhsEzero ) ){
          DEBUG(2, "SIGN(RHS)" << NE << "SIGN(LHS) and at least one is " << NE << 0 << NL);
          _returnAbsurd();
          return true;
        }
        changed |= conju.removeJoin({}, {Constraint::get(factorized.rhs, true)});
        if ( ( sEVI & SIGN::ZERO ) != ( sQUO & SIGN::ZERO ) ){
          if ( sEVI & SIGN::ZERO )
            changed |= conju.removeJoin({}, {Constraint::get(factorized.evidence, true)});
          else changed |= conju.removeJoin({}, {Constraint::get(factorized.quo, true)});
        }
      }
    ...
    if ( ( sEVI == SIGN::UNKNOWN ) || ( sRHS == SIGN::UNKNOWN ) ){
      DEBUG(4, "FIXME: TODO: Test evi - rhs and evi + rhs and check if both have known information\n");
      //TODO: Test evi - rhs and evi + rhs and check if both have known information
    }
    else if ( sEVI & ( SIGN::GTZ | SIGN::LTZ ) ){
      testResult sEviRhs;
      if ( sEVI & sRHS & ( ~SIGN::ZERO ) ){
        sEviRhs = tester->test(factorized.evidence - factorized.rhs);      //rhs and evi have same sign (G?Z or L?Z)
      }
      else{
        sEviRhs = tester->test(factorized.evidence + factorized.rhs);      //rhs and evi have distinct sign (G?Z and L?Z)
      }
      if ( sEviRhs.sign != SIGN::UNKNOWN ){
        if ( sEviRhs.sign & sEVI & ( ~SIGN::ZERO ) ){      //evi - rhs has same sign of evi. Thus |evi| >= |rhs| and |quo| <= 1
          if ( sEviRhs.sign & SIGN::ZERO ){      //|evi| >= |rhs| ===> |quo| <= 1
            switch ( sQUO ){
              case SIGN::UNKNOWN: {
                Conjunction plus = conju, minus = conju;
                plus.removeJoin({}, {Constraint::get(factorized.quo - 1, true)});      //quo = 1
                minus.removeJoin({}, {Constraint::get(factorized.quo + 1, true)});      //quo = -1
                changed |= conju.removeJoin({}, {Constraint::get(factorized.quo, true)});      //quo = 0
              ...
              case SIGN::LTZ: {
                changed |= conju.removeJoin({}, {Constraint::get(-factorized.quo + 1, true)});  //-quo = -1
                DEBUG(4, conju << NL);
                break;
              }
              case SIGN::LEZ: {
                changed |= conju.removeJoin({}, {Constraint::get(-factorized.quo + 1)});  //-1 <= -quo <= 0
                DEBUG(4, conju << NL);
                break;
              }
              case SIGN::GTZ: {
              ...
              case SIGN::GEZ: {
                changed |= conju.removeJoin({}, {Constraint::get(factorized.quo - 1)});  //0 <= quo <= 1
                DEBUG(4, conju << NL);
                break;
              }
              case SIGN::ZERO: {
            ...
            if ( ( ( sQUO != SIGN::UNKNOWN ) && ( !( sQUO & SIGN::ZERO ) ) ) || ( ( sRHS != SIGN::UNKNOWN ) && ( !( sRHS & SIGN::ZERO ) ) ) ){
              DEBUG(4, "|evi| > |RHS|, but (RHS or QUO) " << NE << 0 << NL);
              returnAbsurd();
              return true;
            }
            changed |= conju.removeJoin({}, {Constraint::get(factorized.quo, true), Constraint::get(factorized.rhs, true)});  //quo = 1
          ...
          switch ( sQUO ){
            ...
            case LEZ: {
              if ( sRHS & SIGN::GTZ ){
                changed |= conju.removeJoin({}, {Constraint::get(factorized.quo + factorized.rhs - diff)});
                DEBUG(4,
                    "|evi|" << LE << "|RHS| (" << factorized.rhs << " [" << sRHS << "] ), thus QUO(" << factorized.quo - diff << " [" << sQUO << "] )" << GE << -factorized.rhs << NL << conju << NL);
              }
              else{
                changed |= conju.removeJoin({}, {Constraint::get(factorized.quo - factorized.rhs - diff)});
                DEBUG(4,
            ...
            case GEZ: {
              if ( sRHS & SIGN::GTZ ){
                changed |= conju.removeJoin({}, {Constraint::get(factorized.rhs - factorized.quo + diff)});
                DEBUG(4, "|evi|" << LE << "|RHS| (" << factorized.rhs << " [" << sRHS << "]), thus QUO(" << factorized.quo + diff << " [" << sQUO << "] )" << LE << factorized.rhs << NL << conju << NL);
              }
              else{
                changed |= conju.removeJoin({}, {Constraint::get(-factorized.rhs - factorized.quo + diff)});
                DEBUG(4,
                    "|evi|" << LE << "|RHS| (" << factorized.rhs << " [" << sRHS << "] ), thus QUO(" << factorized.quo + diff << " [" << sQUO << "] )" << LE << -factorized.rhs << NL << conju << NL);
              }
              break;
              default:
              break;
    ...
    if ( ( sQUO == SIGN::UNKNOWN ) || ( sRHS == SIGN::UNKNOWN ) ){
      DEBUG(4, "FIXME: TODO: Test QUO - rhs and QUO + rhs and check if both have known information\n");
      //TODO: Test QUO - rhs and QUO + rhs and check if both have known information
    }
        ...
        if ( sRemRhs.sign & sEVI & ( ~SIGN::ZERO ) ){
          ...
          if ( sRemRhs.sign & SIGN::ZERO ){      //|QUO| >= |rhs| ===> |evi| <= 1
            switch ( sEVI ){
              case SIGN::UNKNOWN: {
                Conjunction plus = conju, minus = conju;
                plus.removeJoin({}, {Constraint::get(factorized.evidence - 1, true)});      //evidence = 1
                minus.removeJoin({}, {Constraint::get(factorized.evidence + 1, true)});      //evidence=-1
                changed |= conju.removeJoin({}, {Constraint::get(factorized.evidence, true)});   //evidence = 0
                DEBUG(4, minus << NL<< conju << NL << plus << NL);
#ifdef SCRIPT_CONTROL
                cout << minus << NL<< conju << NL << plus << NL;
                cout.flush();
              ...
              case SIGN::LTZ: {
                changed |= conju.removeJoin({}, {Constraint::get(-factorized.evidence + 1, true)});   //-evidence = -1
                DEBUG(4, conju << NL);
                break;
              }
              case SIGN::LEZ: {
                changed |= conju.removeJoin({}, {Constraint::get(-factorized.evidence + 1)});   //-1 <= -evidence <= 0
                DEBUG(4, conju << NL);
                break;
              }
              case SIGN::GTZ: {
              ...
              case SIGN::GEZ: {
                changed |= conju.removeJoin({}, {Constraint::get(factorized.evidence - 1)});   //0 <= evidence <= 1
                DEBUG(4, conju << NL);
                break;
              }
              case SIGN::ZERO: {
            ...
            if ( ( ( sEVI != SIGN::UNKNOWN ) && ( !( sEVI & SIGN::ZERO ) ) ) || ( ( sRHS != SIGN::UNKNOWN ) && ( !( sRHS & SIGN::ZERO ) ) ) ){
              DEBUG(4, "|QUO| > |RHS|, but (EVI or QUO) " << NE << 0 << NL);
              returnAbsurd();
              return true;
            }
            changed |= conju.removeJoin({}, {Constraint::get(factorized.evidence, true), Constraint::get(factorized.rhs, true)});  //quo = 1
            DEBUG(4, "|evi| > |RHS|, thus 0 = RHS = QUO\n" << conju << NL);
          }
        }
        else{
          ...
          switch ( sEVI ){
            ...
            case LEZ: {
              if ( sRHS & SIGN::GTZ ){
                changed |= conju.removeJoin({}, {Constraint::get(factorized.evidence + factorized.rhs - diff)});
                DEBUG(4,
                    "|QUO|" << LE << "|RHS| (" << factorized.rhs << sRHS << "), thus evidence(" << factorized.evidence - diff << " [" << sQUO << "])" << GE << -factorized.rhs << NL << conju << NL);
              }
              else{
                changed |= conju.removeJoin({}, {Constraint::get(factorized.evidence - factorized.rhs - diff)});
                DEBUG(4, "|QUO|" << LE << "|RHS| (" << factorized.rhs << sRHS << "), thus evidence(" << factorized.evidence - diff << " [" << sQUO << "])" << GE << factorized.rhs << NL << conju << NL);
              }
              break;
            }
            case GTZ:
            case GEZ: {
              if ( sRHS & SIGN::GTZ ){
                changed |= conju.removeJoin({}, {Constraint::get(factorized.rhs - factorized.evidence + diff)});
                DEBUG(4,
                    "|QUO|" << LE << "|RHS| (" << factorized.rhs << " [" << sRHS << "] ), thus evidence(" << factorized.evidence + diff << " [" << sQUO << "] )" << LE << factorized.rhs << NL << conju << NL);
              }
              else{
                changed |= conju.removeJoin({}, {Constraint::get(-factorized.rhs - factorized.evidence + diff)});
                DEBUG(4,
...
Simplifier::Simplifier(Conjunction &s) :
    conju(s){
}
void Simplifier::run(){
  while ( true ){
    ...
    do{
      ...
      DEBUG(5, conju << NL);
      build_tester();
    } while ( eqs_affinize() || eqs_pattern_matching() || gaussian_replacement() );
    ...
    sanityCheck();
    if ( conju.hasEqs() ){
      DEBUG(0, "Did not eliminate all equalities, convert them to two inequalities\n" << conju);
...
ex Simplifier::compose(const testResult tr, const ex &tested){
  ...
  assertM(tr.distance > (-1.0000000000000000000 + MAX_ERROR), tested << " == " << tr);
  return tested;
}
  ...
  for ( cc eq : conju.eqs ){
    ...
    for ( const ex &p : conju.pars ){
      ...
      for ( int ud = 1; ud <= deg; ud++ ){
        ...
        DEBUG(5, '\t' << eq << " ====> " << term << " * (" << lhs << ") = " << -rhs << NL);
        /*Now we have that term >= 0.
         * To test |term| >= |rhs| we test if term +- rhs >= 0 depending on sign of rhs */
        ...
        switch ( tr.sign ){
          ...
          break;
          case SIGN::GTZ:
            changed |= conju.ineqs.insert(Constraint::get(compose(tr, toTest))).second;
            changed |= conju.eqs.insert(Constraint::get(lhs, true)).second;
...
bool Simplifier::add_affine_planes(){
  ...
  bool changed = false;
  Constraints toJoin;
  for ( auto p1 = conju.pars.begin(), pEnd = conju.pars.end(); p1 != pEnd; p1++ ){
    ...
    for ( auto p2 = std::next(p1); p2 != pEnd; p2++ ){
      ...
      if ( tr.sign != SIGN::UNKNOWN )
        changed |= ( conju.ineqs.insert(Constraint::get(compose(tr, p1Pp2))).second );
      ...
      if ( tr.sign != SIGN::UNKNOWN )
        changed |= ( conju.ineqs.insert(Constraint::get(compose(tr, p1Mp2))).second );
    }
  }

  for ( auto v1 = conju.vars.begin(), vEnd = conju.vars.end(); v1 != vEnd; v1++ ){
    ...
    for ( auto v2 = std::next(v1); v2 != vEnd; v2++ ){
      ...
      if ( tr.sign != SIGN::UNKNOWN )
        changed |= ( conju.ineqs.insert(Constraint::get(compose(tr, v1Pv2))).second );
      ...
      if ( tr.sign != SIGN::UNKNOWN )
        changed |= ( conju.ineqs.insert(Constraint::get(compose(tr, v1Mv2))).second );
    }
    ...
    for ( auto p1 = conju.pars.begin(), pEnd = conju.pars.end(); p1 != pEnd; p1++ ){
      ...
      if ( tr.sign != SIGN::UNKNOWN )
        changed |= ( conju.ineqs.insert(Constraint::get(compose(tr, p1Pv1))).second );
      ...
      if ( tr.sign != SIGN::UNKNOWN )
        changed |= ( conju.ineqs.insert(Constraint::get(compose(tr, p1Mv1))).second );
    }
  }
  return changed;
}
...
bool Simplifier::tightening(){
  ...
  for ( cc c : conju.ineqs ){
    ...
    if ( !r ){
      assertM(false, "Failed to remove an constraint from the duplicated system!!\n");
    }
    testResult tr = other.getTester()->test(c->exp);
    DEBUG(8, "Got " << tr << " when testing the constraint " << c << " against the system\n" << other << NL);
...
bool Simplifier::getRoots(){
  ...
  Conjunction nc(conju.vars, conju.pars, constraintsBySize[minSize], conju.id());
  ...
  while ( minSize < maxSize ){
    ...
    for ( cc c : constraintsBySize[minSize] ){
      ...
      switch ( tr.sign ){
        ...
        case SIGN::ZERO: {
          removed++;
          DEBUG(7, "Remove the constraint " << c << " as it is implied to be " << tr << " by:\n" << nc << NL);
          continue;
        }
        default: {
  ...
  if ( removed ){
    ...
    build_tester();
    DEBUG(4, "Removed " << removed << " redundant constraints.\nThe system is now:" << conju << NL);
    return true;
  }
  return false;
}
...
//Eqs: If m(P) * lhs(P,V) = rhs(P,V), and |m| > |rhs|, that implies that lhs = 0
  ...
  for ( cc c : conju.ineqs ){
    ...
    if ( is_a<power>(lhs) or ( is_a<power>(m_lhs) ) ){
      ...
      if ( neg_is_a_pow ){
        ...
        if ( is_a<power>(lhs) ){
          cerr << "This makes no sense! " << lhs << " is a power, and so it is " << m_lhs << NL;
          throw "ohoh.";
        }
        lhs = move(m_lhs);
      }
      ...
      if ( not neg_is_a_pow ){
        ...
        if ( n.is_odd() ){
          DEBUG(4, "Converting " << c->exp << GE << "0 to " << lhs.op(0) << GE << 0 << NL)
          toRemove.insert(c);
      ...
      for ( const ex &e : LHS ){
        ...
        switch ( tr.sign ){
          ...
          case SIGN::ABSURD: {
            cerr << "The expression " << e << " is both exclusively positive and negative\n";
            returnAbsurd();
            return false;
          }
          case SIGN::LTZ: {
      ...
      if ( simplified ){
        ...
        if ( ( keep ) and ( div != 1 ) ){
          ...
          if ( !divide(LHS, div, newcst) )
            assertM(false, "Computed that " << div << " should divide " << LHS << NL);
          if ( invert )
            newcst = -newcst;
        ...
        for ( const ex &e : lhs ){
          ...
          if ( tr.distance >= df.to_double() ){
            if ( tr.sign & SIGN::GTZ ){
              ...
              else toAdd.insert(Constraint::get(expand(q - 1)));
              toRemove.insert(c);
              break;
            }
          }
...
bool Simplifier::gaussian_replacement(bool usePars){
  ...
  int sysVarsDegree = 999, generatedVarsDegree = 999, sysDegree = 999, generatedDegree = 999;
  ...
  Symbols &toTest = ( ( conju.varsDegree(c->exp) == 0 ) or usePars ) ? conju.pars : conju.vars;
  for ( const ex &v : toTest ){
    if ( quo(c->exp, v, v) == 0 )
      continue;

    int thisSysVarsDegree = 0, thisGeneratedVarsDegree = 0, thisSysDegree = 0, thisGeneratedDegree = 0;
    ...
    DEBUG(5, "=========================\nLooking at variable: " << v << " isolated as " << lhs << " * (" << quoti << ") = " << remain << NL);

    if ( degree(remain, v) != 0 ){
      DEBUG(5, "Can't completely isolate " << v << " as it appears in " << remain << NL);
      continue;
    }
    ...
    if ( !thisCanDivide ){
      if ( negQuo ){
        DEBUG(5, quoti << " does not divide " << remain << " and is negative, setting rhs = " << -remain << ", and quoti = " << -quoti << NL);
        rhs = expand(-remain);
        quoti = expand(-quoti);
      }
      else{
        DEBUG(5, quoti << " does not divide " << remain << ", setting rhs = " << remain << NL);
        rhs = remain;
      }
    }
    else{
      DEBUG(5, quoti << " divides " << remain << ", setting rhs =" << rhs << " and quoti as 1\n");
      quoti = 1;
    }
    ...
    for ( cc other : conju.ineqs ){
      ...
      if ( otherDegree == 0 ){
        thisSysVarsDegree = max(thisSysVarsDegree, conju.varsDegree(other->exp));
        thisSysDegree = max(thisSysDegree, conju.polynomialDegree(other->exp));
        continue;
      }

      if ( ( deg != 1 ) and ( ( otherDegree % deg != 0 ) or ( degree(quo(other->exp, lhs, lhs), v) != 0 ) ) ){
        DEBUG(5, "Can't use " << lhs << " as it can't be properly isolated in " << other << NL);
        canReplace = false;
        break;
      }
      DEBUG(5, "Replacing " << v << " = (" << rhs << ") / (" << quoti << ") on " << other << NL);

      ex newcst = expand(subs(other->exp * power(quoti, otherDegree), v == ( rhs / quoti )));
      bool ok = true;
      if ( is_a<add>(newcst) ){
      ...
      thisGeneratedVarsDegree = max(thisGeneratedVarsDegree, conju.varsDegree(newcst));
      thisGeneratedDegree = max(thisGeneratedDegree, conju.polynomialDegree(newcst));
      thisSysVarsDegree = max(thisSysVarsDegree, thisGeneratedVarsDegree);
      thisSysDegree = max(thisSysDegree, thisGeneratedDegree);

      DEBUG(4, "Replacing " << v << " on " << other << " get " << newcst << GE << "0\n");
    ...
    for ( cc other : conju.eqs ){
      ...
      ex newcst = expand(subs(other->exp * power(quoti, otherDegree), v == ( rhs / quoti )));
      assertM(newcst.degree(v) == 0, "After replacing we still have the replaced variable\n");
      bool ok = true;
      if ( is_a<add>(newcst) ){
        for ( const ex &e : newcst ){
          if ( e.denom() != 1 ){
      ...
      thisSysVarsDegree = max(thisSysVarsDegree, thisGeneratedVarsDegree);
      thisSysDegree = max(thisSysDegree, thisGeneratedDegree);
      DEBUG(4, "Replacing " << v << " on " << other << " get " << newcst << " = 0\n");
      thisToRemove.insert(other);
      thisEqs.insert(newcst);
    }

    DEBUG(5, "Testing if we should take this replacement as best option!\n");
    DEBUG(5, "Could we perform the replacement everywhere? " << canReplace << NL);
    if ( !canReplace )
      continue;

    DEBUG(5, "The generated system has a degree in the variables" << LE << sysVarsDegree << "? " << thisSysVarsDegree << NL);
    if ( sysVarsDegree < thisSysVarsDegree )
      continue;

    if ( sysVarsDegree == thisSysVarsDegree ){
      DEBUG(5, "The generated constraints have a degree in the variables" << LE << generatedVarsDegree << "? " << thisGeneratedVarsDegree << NL);
      if ( generatedVarsDegree < thisGeneratedVarsDegree )
        continue;

      if ( generatedVarsDegree == thisGeneratedVarsDegree ){
        DEBUG(5, "The generated system has a maximum degree" << LE << sysDegree << "? " << thisSysDegree << NL);
        if ( sysDegree < thisSysDegree )
          continue;

        if ( sysDegree == thisSysDegree ){
          DEBUG(5, "The generated constraints have a maximum degree" << LE << generatedDegree << "? " << thisGeneratedDegree << NL);
          if ( generatedDegree < thisGeneratedDegree )
            continue;

          if ( generatedDegree == thisGeneratedDegree ){
            DEBUG(5, "When replacing the variable, did we perform a 'cleaner isolation'? " << ( thisCleanIsolation > cleanIsolation ) << NL);
            if ( cleanIsolation > thisCleanIsolation )
              continue;

            if ( cleanIsolation == thisCleanIsolation ){
              DEBUG(5, "Did the quotient better divide the remaining expression? " << ( (not canDivide) and thisCanDivide ) << NL);
    ...
    DEBUG(4,
        "Best replacement so far:\n\tReplacing: " << eliminated << "\nObtaining:\n\tSystem degree in variables:" << sysVarsDegree << "\n\tNew constraints max degree in variables:" << generatedVarsDegree << "\n\tSystem degree in variables:" << sysVarsDegree << "\n\tSystem degree:" << sysDegree << "\n\tGenerated constraints max degree: " << generatedDegree << "\n\tCould completely isolate the variable? " << cleanIsolation << "\n\tThe variable quotient did divide the remaining of the equality ? " << canDivide << NL);
  }
  ...
  if ( eliminated != 0 ){
    ...
    if ( ( conju.varsDegree(c->exp) == 0 ) or usePars ){
      DEBUG(4, "Replaced " << eliminated << " everywhere else, converting " << c << " to 2 inequalities!\n");
...
void Simplifier::Motzkin(){
  ...
  DEBUG(0, "Selecting variable " << target << ", with degree " << targetDegree << " for elimination");
  ...
  if ( not ( not_lifting or ( targetDegree < min(2u, lifting_target_degree) ) ) ){
    ...
    DEBUG(0, "Performing Motzkin by lifting " << target_here << " as the target variable");
  }
  ...
  for ( cc c : conju.ineqs ){
    ...
    if ( it != sign_cache.end() ){
      ...
      if ( it != sign_cache.end() ){
        res = it->second;
        DEBUG(0, "We have cached that the sign of minus (" << q << ") is " << res << NL);
        switch ( res.sign ){
          case SIGN::GEZ:
            res.sign = SIGN::LEZ;
          break;
    ...
    DEBUG(0, c << " contains " << target_here << " with coefficient " << q << " which is of sign: " << res);
    switch ( res.sign ){
      case SIGN::ABSURD: {
...
//  const ex powerRemoval = GiNaC::pow(2 * 3 * 5 * 7 * 9 * 11 * 13 * 17 * 19 * 23, 4);
  for ( cMap::const_iterator lbIt = lowerBounds.begin(); lbIt != lowerBounds.end(); lbIt++ ){
    for ( cc lb : lbIt->second ){
      ...
      for ( cMap::const_iterator ubIt = upperBounds.begin(); ubIt != upperBounds.end(); ubIt++ ){
        for ( cc ub : ubIt->second ){
          ...
          newMaxDegree = std::max((unsigned) ( abs(degree(newCstr, target_here)) ), newMaxDegree);
          DEBUG(3,
              "We have that:\n" << "\tlower bound:" << lb << " | written as: " << target_here << GE << '(' << expand(-loRemai) << ") / (" << loCoeff << ")\n" << "\tupper bound:" << ub << " | written as: " << target_here << LE << '(' << expand(-upRemai) << ") / (" << upCoeff << ")\n" << "\tgiving: " << newCstr << GE << 0 << NL);
  ...
  if ( zero.size() > 0 ){
    ...
    for ( cc c : zero ){
      ...
      ex newCst = subs(subs(subs(c->exp, target_here == 0), q == 0), expand(-q) == 0);
      ...
      for ( cc e : remaining )
        nr.insert(Constraint::get(( subs(subs(e->exp, q == 0), expand(-q) == 0) )));
  ...
  DEBUG(2, "New constraints have same target |" << target_here << "| at maximum degree of " << newMaxDegree << NL);
  lifting_target_degree = min(lifting_target_degree, targetDegree - 1);
  DEBUG(0, "Target is to lower " << target_here << " to the degree " << lifting_target_degree);

  if ( not_lifting )
    not_lifting--;

  for ( cMap::const_iterator it = lowerBounds.begin(); it != lowerBounds.end(); it++ ){
    if ( it->first < max((unsigned) 1, lifting_target_degree) )
      remaining.insert(it->second.begin(), it->second.end());
  }

  for ( cMap::const_iterator it = upperBounds.begin(); it != upperBounds.end(); it++ ){
    if ( it->first < max((unsigned) 1, lifting_target_degree) )
      remaining.insert(it->second.begin(), it->second.end());
  }
...
void Simplifier::selectTarget(){
  ...
  for ( const ex &s : conju.vars ){
    ...
    for ( const cc c : conju.ineqs ){
      ...
      if ( varDegree == degree and ( nonNumeric == nonNum ) and ( exps >= numExps ) ){
        newVar = false;
        break;
      }
    }
