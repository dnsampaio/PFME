...
Conjunction::Conjunction(const Symbols &v, const Symbols &p, const Constraints &e, const Constraints &i, const unsigned id) :
  ...
  for ( cc c : ineqs ){
    if ( ineqs.find(c->getNot()) != ineqs.end() ){
      DEBUG(1, "This system contains to inverse constraints:" << c << " AND " << c->getNot() << NL);
      makeFalse();
      return;
    }
  }
...
Conjunction::Conjunction(const Symbols &v, const Symbols &p, const Constraints &constr, const unsigned id) :
...
Conjunction::Conjunction(string &in) :
  ...
  string constraintsStr = regex_replace(regex_replace(regex_replace(in.substr(s + 1), ge, ">="), le, "<="), rg, "");
  string tst = constraintsStr;
  for (auto & c: tst) c = toupper(c);
  if ( tst == "FALSE" ){
    makeFalse();
    return;
  }
  ...
  for ( s = constraintsStr.find_first_of('&'); s != string::npos; s = constraintsStr.find_first_of('&') ){
    string newConstraintStr = constraintsStr.substr(0, s);
    constraintsStr = constraintsStr.substr(s + 1);
    DEBUG(10, "Obtaining constraint from the string: " << newConstraintStr << NL);
    ...
    if ( c->eq )
      ...
      if ( ineqs.find(c->getNot()) != ineqs.end() ){
        cerr << "Holding " << c << " and " << c->getNot() << " makes the system empty.\n";
        makeFalse();
        return;
      }
      ineqs.insert(c);
...
bool Conjunction::removeJoin(const Constraints &toRemove, const Constraints &toJoin){
  if ( *this == absurd )
    return false;
  ...
  for ( const cc c : toJoin ){
    ...
    else changed |= ineqs.insert(c).second;
  }
  if ( empty() )
    *this = obvious;
...
void Conjunction::variablesStr2set(string &str, Symbols &s){
  ...
  for ( size_t p = str.find_first_of(','); p != string::npos; p = str.find_first_of(',') ){
